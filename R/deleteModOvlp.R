deleteModOvlp <- function(n_modules,networks,meetMin_cutoff){
  
  
  #library(Matrix);
  #lname=load("combine_nmodule(0.6_0.01).RData");
  
  moduleLen = sapply( n_modules, function(x) length(x$members) )
  n_modules = n_modules[order( moduleLen, decreasing=T )]
  ...
  # for(i in 1:length(n_modules)){
  #  if (length(n_modules[[i]]$members)>=5){
  ...
  while(index){
    ...
    for(i in (start+1):length(n_modules)){
      ...
      temp2 = n_modules[[i]]$members
      #temp2 = unique(union(n_modules[[start]]$members,n_modules[[i]]$members));
      if (length(temp1)>meetMin_cutoff*length(temp2)){
        tempcount=c(tempcount,i);
      }
    }
    if(length(tempcount)>0){
      n_modules[tempcount]<-NULL;
    }
    
    start=start+1;
    
    if(start<length(n_modules)){
      index=1;
    }else{
      index=0;
    }
  ...
  print(  paste( length(n_modules)," modules were detected after refinement",sep=""   ) );
